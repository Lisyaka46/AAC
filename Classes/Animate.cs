namespace AAC.Classes
{
    ...
    public class AnimationDL
    {
        ...
        public static class Animate
        {
            ...
            /// <summary>
            /// Главный класс анимации нумерационного параметра
            /// </summary>
            public static class AnimNumeration
            {
                /// <summary>
                /// Массив активных имён анимации нумерационного параметра
                /// </summary>
                public static List<string> ActiveElementsNumerationAnimation = [];

                /// <summary>
                /// Массив активных имён завершающих своё действие анимации нумерационного параметра
                /// </summary>
                public static List<string> StopingNumerationAnimation = [];

                /// <summary>
                /// Класс инструкции анимации нумерационного параметра
                /// </summary>
                public class Instr_AnimNumeration
                {
                    /// <summary>
                    /// Стартовое значение анимации нумерационного параметра
                    /// </summary>
                    public float StartNumber { get; private set; }

                    /// <summary>
                    /// Конечное значение анимации нумерационного параметра
                    /// </summary>
                    public float EndNumber { get; private set; }

                    /// <summary>
                    /// Стиль конечного числа параметра
                    /// </summary>
                    public StyleEndNum StyleEndNumber { get; private set; }

                    /// <summary>
                    /// Инициализировать объект инструкции к анимации нумерационного параметра
                    /// </summary>
                    /// <param name="StartNum">Стартовое значение параметра</param>
                    /// <param name="EndNum">Конечное значение параметра</param>
                    /// <param name="styleEnd">Конечный стиль числа параметра визуализации</param>
                    public Instr_AnimNumeration(float StartNum, float EndNum, StyleEndNum styleEnd = StyleEndNum.INT)
                    {
                        StartNumber = StartNum;
                        EndNumber = EndNum;
                        StyleEndNumber = styleEnd;
                    }

                    /// <summary>
                    /// Инициализировать анимацию нумерационного параметра
                    /// </summary>
                    /// <param name="Object">Объект анимации</param>
                    public void InitNumerationAnimation(dynamic Object)
                    {
                        ThreadStart Animation = new(() =>
                        {
                            if ((int)StartNumber == (int)EndNumber) return;

                            if (ActiveElementsNumerationAnimation.Contains(Object.Name))
                            {
                                StopingNumerationAnimation.Add(Object.Name);
                                StartNumber = Convert.ToInt32(Object.Text);
                            }

                            int num = (int)StartNumber;
                            List<int> NumberList = [ num ];
                            while (Math.Abs(num - EndNumber) > 3f)
                            {
                                if (StartNumber < EndNumber) num += (int)(Math.Abs(num - EndNumber) / 3f);
                                else num -= (int)(Math.Abs(num - EndNumber) / 3f);
                                NumberList.Add(num);
                            }
                            NumberList.Add((int)EndNumber);
                            ActiveElementsNumerationAnimation.Add(Object.Name);

                            Object.Text = ((int)StartNumber).ToString();
                            while (NumberList.Count > 0)
                            {
                                Object.Text = (NumberList[0]).ToString();
                                NumberList.RemoveAt(0);
                                if (StopingNumerationAnimation.Contains(Object.Name))
                                {
                                    StopingNumerationAnimation.Remove(Object.Name);
                                    ActiveElementsNumerationAnimation.Remove(Object.Name);
                                    return;
                                }
                                Thread.Sleep(20);
                            }
                            if (StyleEndNumber == StyleEndNum.FLOAT) Object.Text += ".0";
                            StopingNumerationAnimation.Remove(Object.Name);
                            ActiveElementsNumerationAnimation.Remove(Object.Name);
                        });
                        Object.Text = StartNumber.ToString();
                        new Thread(Animation).Start();
                    }
                }

                /// <summary>
                /// Стили конечных значений анимации нумерационного параметра
                /// </summary>
                public enum StyleEndNum
                {
                    /// <summary>
                    /// Конечное число представляет собой число int
                    /// </summary>
                    INT = 0,

                    /// <summary>
                    /// Конечное число представляет собой число float
                    /// </summary>
                    FLOAT = 1
                }
            }
